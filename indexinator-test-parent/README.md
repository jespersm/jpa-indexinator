...
- `indexinator-test-sb35/`
  - Source-less runner module for Spring Boot 3.5 + Java 17.
  - Uses `build-helper-maven-plugin` to compile/run shared sources from `indexinator-test-parent/src`.
- `indexinator-test-sb4/`
...
Each runner module (`indexinator-test-sb35`, `indexinator-test-sb4`) adds these shared directories:

- `../indexinator-test-parent/src/main/java` as main sources
- `../indexinator-test-parent/src/test/java` as test sources
- `../indexinator-test-parent/src/test/resources` as test resources

## Run Commands

```bash
# SB 3.5 line (Java 17+)
mvn -pl indexinator-test-sb35 -am test

# SB 4 line (Java 21+)
mvn -pl indexinator-test-sb4 -am test

# Compile-only check for SB4 (if Docker is unavailable)
mvn -pl indexinator-test-sb4 -am clean test -DskipTests
```
